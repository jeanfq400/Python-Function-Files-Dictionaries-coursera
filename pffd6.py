...
#a while loop that only stops once the element of the list is the number 7.
...
#Write a function called stop_at_z that iterates through a list of strings. Using
#a while loop, append each string to a new list until the string that appears is “z”.
...
#that it does the same thing, but using a while loop instead of a for loop.
...
#regardless of where the loop stops. (i.e., if it stops on the 32nd element, the first
#10 are returned. If “bye” is the 5th element, the first 4 are returned.) If you want to make
